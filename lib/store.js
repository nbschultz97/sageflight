...
// Cached per-log analysis metrics (for the trends view — full frame decode
// takes seconds per log, so it runs once and persists).
function saveBlackboxAnalysis(name, metrics) {
  ensureDirs();
  const safe = sanitizeId(name);
  fs.writeFileSync(path.join(BLACKBOX_DIR, `${safe}.analysis.json`),
    JSON.stringify({ name: safe, analyzedAt: new Date().toISOString(), ...metrics }, null, 2), 'utf8');
}

function readBlackboxAnalysis(name) {
  const safe = sanitizeId(name);
  const file = path.join(BLACKBOX_DIR, `${safe}.analysis.json`);
  if (!safe || !fs.existsSync(file)) return null;
...
module.exports = {
  ...
  saveBlackboxAnalysis, readBlackboxAnalysis,
