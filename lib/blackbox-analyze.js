...
// ---------- Cross-flight trends (the tune coach's data layer) ----------

// Compact one number per concern from a cached metricsForLlm() blob, so a
// craft's flights can sit side by side on a timeline.
function trendPoint(m) {
  if (!m) return null;
  const ax = (list, axis) => (list || []).find(g => g.axis === axis) || {};
  const peakOf = (g) => (g.peaks && g.peaks[0]) || null;
  const rollPeak = peakOf(ax(m.gyro, 'roll'));
  const step = (axis, field) => {
    const s = (m.stepResponse || []).find(r => r.axis === axis);
    return s ? s[field] ?? null : null;
  };
  return {
    durationSec: m.durationSec ?? null,
    rms: {
      roll: ax(m.gyro, 'roll').rmsDegS ?? null,
      pitch: ax(m.gyro, 'pitch').rmsDegS ?? null,
      yaw: ax(m.gyro, 'yaw').rmsDegS ?? null,
    },
    peakHz: rollPeak ? rollPeak.hz : null,
    peakRatio: rollPeak ? rollPeak.ratioToFloor : null,
    riseMs: { roll: step('roll', 'riseMs'), pitch: step('pitch', 'riseMs') },
    overshootPct: { roll: step('roll', 'overshootPct'), pitch: step('pitch', 'overshootPct') },
    motorImbalance: m.motorImbalance ?? null,
  };
}

// Group log metadata by craft (fallback: board target, then "unnamed"),
// oldest first — trends read left to right.
function groupLogsForTrends(logs) {
  const groups = new Map();
  for (const l of logs || []) {
    const key = l.craft || l.board || 'unnamed craft';
...
module.exports = { analyzeBuffer, metricsForLlm, trendPoint, groupLogsForTrends };
