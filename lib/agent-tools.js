...
const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    function: {
      name: 'search_catalog',
      description: 'Search the cots-catalog hardware database for component specs: motors (kv, thrust, current), ESCs (rating, firmware), flight controllers, batteries, VTX, receivers. Use for "what are the specs of X" or "will this motor work with this ESC" questions. Returns null if no catalog is available — that is fine, Sageflight works standalone.',
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'Component search, e.g. "2207 motor 1950kv" or "50A 4-in-1 ESC"' },
        },
        required: ['query'],
      },
    },
  },
  {
    ...
    function: {
      name: 'search_docs',
      description: 'Search the local Betaflight documentation index for authoritative information. Use this BEFORE answering questions about CLI settings, features, tuning parameters, or procedures — cite what it returns instead of relying on memory. Returns the most relevant documentation excerpts with their source files.',
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'What to look up, e.g. "dshot bidir rpm filter requirements"' },
        },
        required: ['query'],
      },
    },
...
// deps: { detectFC, scanFC, withCli, runExclusive, store, forensic, connection, searchDocs, searchCatalog }
function createToolExecutor(deps) {
  const { detectFC, scanFC, withCli, runExclusive, store, forensic, connection, searchDocs, searchCatalog } = deps;
  ...
  const handlers = {
    ...
    async search_catalog({ query } = {}) {
      if (!searchCatalog) return { error: 'catalog search not available' };
      if (!query) return { error: 'query is required' };
      const results = searchCatalog(String(query));
      if (results == null) {
        return { results: null, note: 'No cots-catalog available. Sageflight works standalone — answer from docs/general knowledge and say specs are unverified.' };
      }
      return { results };
    },
    async search_docs({ query } = {}) {
      if (!searchDocs) return { error: 'docs search not available' };
      if (!query) return { error: 'query is required' };
      const results = await searchDocs(String(query));
      if (results == null) {
        return { results: null, note: 'Docs index not built yet — tell the user to click "Build docs index" in the AI Assistant tab (needs internet once).' };
      }
      return { results };
    },
    async get_loadout() {
